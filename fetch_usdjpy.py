...
elif response.status\_code == 402:
\# Payment required or quota exceeded; fallback to no events
print(f"Warning: FMP API access issue ({response.status\_code}); defaulting to no events")
events = \[]
else:
\# Other HTTP errors
print(f"Warning: FMP calendar fetch failed with status {response.status\_code}; defaulting to no events")
events = \[]
...
# 3) Fetch intraday data (1h) and compute ATR(14) + pivots (1h) and compute ATR(14) + pivots
...
Today's high-volatility report: {high\_vol\_event}
...
{{
"direction": "Long|Short|Neutral",
"stop\_loss": {stop\_loss:.4f},
"take\_profit\_1": {take\_profit\_1:.4f},
"take\_profit\_2": {take\_profit\_2:.4f},
